# https://github.com/azorg/sx127x_esp/tree/master

# -*- coding: UTF8 -*-
# Micropython ESP8266/ESP32 driver for Semtech SX127x famaly chips
# Author: Alex Zorg <azorg(at)mail.ru>
# Licenced by GPLv3
# Modified by sendust 2024/2/24
# Raspberry pi PICO Wireless Tally project
#
...
class RADIO:
    def __init__(self,
                 ...
                 gpio = {'led':    25,    # blue LED GPIO number on board   2 
                         'reset':  9,    # reset pin from GPIO5 (or may be None)   5
                         'dio0':   2,    # DIO0 line to GPIO4    4
                         'cs':     5,   # SPI CS   15
                         'sck':    6,   # SPI SCK   14
                         'mosi':   7,   # SPI MOSI   13
                         'miso':   4},  # SPI MISO    12
        ...
        self.spi = SPI(0, 10_000_000, sck=Pin(gpio['sck']), mosi=Pin(gpio['mosi']), miso=Pin(gpio['miso']))
        self.spi.init()
    ...
    def led(self, on=True):
        ...
        self.pin_led_status = not LED_ON ^ on

    def led_toggle(self):
        self.pin_led_status = not self.pin_led_status
        self.pin_led.value(self.pin_led_status)
        sleep_ms(8)
    ...
    def rxCalibrate(self):
        ...
        if self._mode:
            ...
            while (self.readReg(REG_IMAGE_CAL) & 0x20): # `ImageCalRunning`
                self.led_toggle()
                if time() - tm_start > 3:
                    print("Error waiting RSSI and IQ callibration (FSK/OOK)")
                    break
    ...
    def send(self, string, fixed=False):
        ...
        if self._mode == 0: # LoRa mode
            ...
            # wait `TxReady` (bit 5 in `RegIrqFlags1`)
    ...
    def send_lora(self, string, fixed=False):
        """send packet (LoRa/FSK/OOK)"""
